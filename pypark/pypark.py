...
def run():
    ...
    while run:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if left_button:
                    ...
                    tile_vector2d = camera.get_corresponding_tile(mouse_pos[0], mouse_pos[1])
        ...
        # draw text top left
        font_surface1 = f.render("x: %s, y: %s" % pygame.mouse.get_pos(), True, (255, 120, 255), (0,0,0))
        screen.blit(font_surface1, (0, 0))

        tile_bounds = camera.get_tile_bounds()
        font_surface2 = f.render("viewing - top left: %s, bottom right: %s" % (tile_bounds.topleft, tile_bounds.bottomright), True, (255, 120, 255), (0,0,0))
        screen.blit(font_surface2, (0, font_surface1.get_height()))

        font_surface3 = f.render("mouse last clicked - x: %s, y: %s" % (mouse_pos[0], mouse_pos[1]), True, (255, 120, 255), (0,0,0))
        screen.blit(font_surface3, (0, font_surface2.get_height()+font_surface1.get_height()))

        font_surface4 = f.render("mouse tile last clicked - x: %s, y: %s" % (x, y), True, (255, 120, 255), (0,0,0))
        screen.blit(font_surface4, (0, font_surface1.get_height()*3))

        hover_tile = camera.get_corresponding_tile(*pygame.mouse.get_pos())
        font_surface5 = f.render("mouse tile hover - x: %s, y: %s" % (hover_tile.x, hover_tile.y), True, (255, 120, 255), (0,0,0))
